...
struct Ragebot {
  ...
  void StartAiming() {
    ...
    if (Features::Ragebot::AimMethod == 0) { // Memory
      ...
      if (!Features::Ragebot::AutoShoot) { Myself->SetViewAngle(VectorDesiredAngles); }
    ...
    if (Features::Ragebot::AimMethod == 1) { // Mouse
      ...
      /*if (totalPitchIncrementInt == 0 && totalYawIncrementInt == 0)
        return;*/
      if (!Features::Ragebot::AutoShoot) { X11Display->MoveMouse(totalYawIncrementInt, totalPitchIncrementInt); }
      if (Features::Ragebot::AutoShoot) {
  ...
  bool GetAngleToTarget(Player *Target, QAngle &Angle) {
    ...
    if (!Features::Ragebot::ClosestHitbox) { TargetPosition = Target->GetBonePosition(Features::Ragebot::Hitbox); }
    if (Features::Ragebot::ClosestHitbox) { TargetPosition = Target->GetBonePosition(static_cast<HitboxType>(GetBestBone(Target))); }
    ...
    if (Features::Ragebot::PredictBulletDrop && Features::Ragebot::PredictMovement) { return Resolver::CalculateAimRotationNew(CameraPosition, TargetPos, TargetVelocity, Myself->WeaponProjectileSpeed, Myself->WeaponProjectileScale, Features::Ragebot::PreditcionAmount, Angle); } else if (Features::Ragebot::PredictMovement) { return Resolver::CalculateAimRotationNew(CameraPosition, TargetPos, Vector3D(0, 0, 0), Myself->WeaponProjectileSpeed, Myself->WeaponProjectileScale, Features::Ragebot::PreditcionAmount, Angle); } else if (Features::Ragebot::PredictBulletDrop) { return Resolver::CalculateAimRotation(CameraPosition, TargetPos, TargetVelocity, Myself->WeaponProjectileSpeed, Angle); }
  ...
  bool IsValidTarget(Player *target) {
    if (target == nullptr || !target->IsValid() || !target->IsCombatReady() || !target->IsVisible || !target->IsHostile || target->Distance2DToLocalPlayer < Conversion::ToGameUnits(Features::Ragebot::MinDistance) || target->Distance2DToLocalPlayer > Conversion::ToGameUnits(FinalDistance))
      return false;
    return true;
  }
