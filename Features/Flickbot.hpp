...
struct Flickbot {
  ...
  void StartAiming() {
    ...
    if (Features::Flickbot::FlickbotMethod == 0) { // Mouse
      ...
      if (FlickbotDelta.x == 0 && FlickbotDelta.y == 0)
        return;
  ...
  bool GetAngleToTarget(Player *Target, QAngle &Angle) {
    ...
    if (!Features::Flickbot::ClosestHitbox) { TargetPosition = Target->GetBonePosition(Features::Flickbot::Hitbox); }
    if (Features::Flickbot::ClosestHitbox) { TargetPosition = Target->GetBonePosition(static_cast<HitboxType>(GetBestBone(Target))); }
    ...
    if (Myself->WeaponProjectileSpeed > 1.0f) { if (Features::Flickbot::PredictBulletDrop && Features::Flickbot::PredictMovement) { return Resolver::CalculateAimRotationNew(CameraPosition, TargetPos, TargetVelocity, Myself->WeaponProjectileSpeed, Myself->WeaponProjectileScale, Features::Flickbot::PreditcionAmount, Angle); } else if (Features::Flickbot::PredictBulletDrop) { return Resolver::CalculateAimRotationNew(CameraPosition, TargetPos, Vector3D(0, 0, 0), Myself->WeaponProjectileSpeed, Myself->WeaponProjectileScale, Features::Flickbot::PreditcionAmount, Angle); } else if (Features::Flickbot::PredictMovement) { return Resolver::CalculateAimRotation(CameraPosition, TargetPos, TargetVelocity, Myself->WeaponProjectileSpeed, Angle); } }
  ...
  bool IsValidTarget(Player *target) {
    if (target == nullptr || !target->IsValid() || !target->IsCombatReady() || !target->IsVisible || !target->IsHostile || target->Distance2DToLocalPlayer < Conversion::ToGameUnits(Features::Flickbot::MinDistance) || target->Distance2DToLocalPlayer > Conversion::ToGameUnits(FinalDistance))
      return false;
    return true;
  }
