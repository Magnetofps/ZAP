...
struct LocalPlayer {
  ...
  void ResetPointer() { BasePointer = 0; }
  ...
  bool IsValid() { return BasePointer != 0; }
  ...
  void SetQAngle(QAngle ViewAngle) { Memory::Write<QAngle>(BasePointer + OFF_VIEW_ANGLES, NormalizeQAngle(ViewAngle)); }
  ...
  void SetViewAngle(Vector2D ViewAngle) { Memory::Write<Vector2D>(BasePointer + OFF_VIEW_ANGLES, NormalizeVectorAngles(ViewAngle)); }
